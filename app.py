...
def registerCriminal():
    if request.method == 'POST':
        ...
        #save im0 croped image
        save_path = 'static/CombinedFace/'
        imagename=save_path+name+"_"+national_id
        completePath=imagename+"_0.jpg"
        cv2.imwrite(completePath,img0)
        feature=Features(completePath)
        feature_path = Path("./static/Features") / (Path(completePath).stem + ".npy")
        np.save(feature_path,feature)
        #save im2 croped image
        save_path = 'static/CombinedFace/'
        imagename=save_path+name+"_"+national_id
        completePath=imagename+"_2.jpg"
        cv2.imwrite(completePath,img2)
        feature=Features(completePath)
        feature_path = Path("./static/Features") / (Path(completePath).stem + ".npy")
        np.save(feature_path,feature)
        #save im3 croped image
        save_path = 'static/CombinedFace/'
        imagename=save_path+name+"_"+national_id
        completePath=imagename+"_3.jpg"
        cv2.imwrite(completePath,img3)
        feature=Features(completePath)
        feature_path = Path("./static/Features") / (Path(completePath).stem + ".npy")
        np.save(feature_path,feature)
...
# Route for findCriminal.html by ID
@app.route('/findCriminalbyID',methods=['GET', 'POST'])
def findCriminalbyID():
    if request.method == 'POST':
        id=request.form['national_id']
        user = User.query.filter_by(national_id=id).first()
        return  render_template('findCriminal.html',userbyID=user)
    else:
        return render_template('findCriminal.html')
